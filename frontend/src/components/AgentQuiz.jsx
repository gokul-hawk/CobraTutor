    ...
    useEffect(() => {
        const reportToAgent = async () => {
            if (currentPhase === 'results' && results) {
                ...
                // const isSuccess = !results.all_failed_topics || results.all_failed_topics.length === 0; // Remved check
                const hasNext = results.next_quizzes && results.next_quizzes.length > 0;

                // If no next quizzes (whether success or fail), report to agent
                if (!hasNext) {
                    try {
                        const res = await axios.post(
                        ...
                        if (res.data.action) {
                            setTimeout(() => {
                                ...
                                if (res.data.action.view === 'tutor') navigate('/agent-tutor', { state: { initialMessage: res.data.action.data?.initialMessage || res.data.reply } }); // Pass reply
            ...
            <div className="max-w-2xl w-full bg-white rounded-2xl shadow-xl overflow-hidden p-8 text-center">
                ...
                {(!results.next_quizzes || results.next_quizzes.length === 0) ? (
                    <div className="animate-fade-in-up">
                        ...
                        <h3 className="text-2xl font-semibold text-green-800 mb-2">Diagnostic Submitted</h3>
                        <p className="text-gray-600 mb-8">{results.message || "Reporting results to agent..."}</p>
